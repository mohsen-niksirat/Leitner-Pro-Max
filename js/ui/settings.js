...
h+='<div><label style="display:block;font-size:.85rem;color:var(--text2);margin-bottom:4px">تعداد worker همزمان (غنی‌سازی)</label><select class="input" id="setVfConcurrency"><option value="4"'+((s.vfConcurrency||6)===4?' selected':'')+'>۴ — کم‌مصرف</option><option value="6"'+((!s.vfConcurrency||s.vfConcurrency===6)?' selected':'')+'>۶ — پیش‌فرض</option><option value="8"'+(s.vfConcurrency===8?' selected':'')+'>۸ — سریع</option><option value="12"'+(s.vfConcurrency===12?' selected':'')+'>۱۲ — خیلی سریع</option></select></div>';
