...
h+='<div><label style="display:block;font-size:.85rem;color:var(--text2);margin-bottom:4px">تعداد worker همزمان (غنی‌سازی)</label><select class="input" id="setVfConcurrency"><option value="4"'+((s.vfConcurrency||10)===4?' selected':'')+'>۴ — کم‌مصرف</option><option value="6"'+(s.vfConcurrency===6?' selected':'')+'>۶ — متوسط</option><option value="8"'+(s.vfConcurrency===8?' selected':'')+'>۸ — سریع</option><option value="10"'+((!s.vfConcurrency||s.vfConcurrency===10)?' selected':'')+'>۱۰ — پیش‌فرض</option><option value="12"'+(s.vfConcurrency===12?' selected':'')+'>۱۲ — خیلی سریع</option></select></div>';
...
document.getElementById('setVfConcurrency').onchange=function(e){S.settings.vfConcurrency=parseInt(e.target.value)||6;save()};
